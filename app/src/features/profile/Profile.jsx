import { useState, useEffect, useRef } from "react";
import { useNavigate } from "react-router-dom";
import { useProfile } from "./useProfile";
import { apiGet, apiPatch, apiUpload } from "../../api/client";
import { getProfile, setProfile } from "../../lib/session";
import { BODY_TYPES, GENDERS } from "../../lib/profileConstants.jsx";

const STYLE_LABELS = {
  casual: "Casual",
  formal: "Formal",
  minimalist: "Minimalist",
  bohemian: "Bohemian",
  streetwear: "Street",
  romantic: "Romantic",
  sporty: "Sporty",
  classic: "Classic",
};

const PALETTE_COLORS = {
  warm: ["#C9A961", "#C2563A", "#9A3E26", "#EBE3D4", "#D4C4A0"],
  cool: ["#8C8273", "#9BA3A8", "#5B6A7A", "#D0D5DA", "#EFF1F3"],
  earthy: ["#5B6A3F", "#3F4B2A", "#8B7355", "#A0895A", "#D4C4A0"],
  mono: ["#1A1612", "#3A322A", "#8C8273", "#D4CBB8", "#FBF8F1"],
  bold: ["#E63946", "#4361EE", "#2DC653", "#FF9800", "#9B59B6"],
};

const PALETTE_LABELS = {
  warm: "Warm neutrals",
  cool: "Cool neutrals",
  earthy: "Earthy greens",
  mono: "Monochrome",
  bold: "Bold & bright",
};

const BODY_LABELS = {
  hourglass: "Hourglass",
  pear: "Pear",
  apple: "Apple",
  rectangle: "Rectangle",
  inverted: "Inverted Triangle",
};

// Radar chart axes map to style preference keys
const RADAR_AXES = [
  { label: "Casual", keys: ["casual", "sporty"] },
  { label: "Formal", keys: ["formal", "classic"] },
  { label: "Creative", keys: ["bohemian", "romantic"] },
  { label: "Classic", keys: ["classic", "minimalist"] },
  { label: "Sporty", keys: ["sporty", "streetwear"] },
];

function RadarChart({ prefs = [] }) {
  const S = 200,
    cx = 100,
    cy = 105,
    maxR = 68;

  const scores = RADAR_AXES.map((a) => {
    if (prefs.length === 0) return 45;
    const hit = a.keys.filter((k) => prefs.includes(k)).length;
    return hit > 0 ? 72 + hit * 6 : 22;
  });

  const angles = RADAR_AXES.map((_, i) => -Math.PI / 2 + (i * 2 * Math.PI) / 5);

  function pt(angle, value, r = maxR) {
    const rv = (value / 100) * r;
    return [cx + rv * Math.cos(angle), cy + rv * Math.sin(angle)];
  }

  const scorePoints = angles.map((a, i) => pt(a, scores[i]));

  return (
    <svg
      width={S}
      height={S}
      viewBox={`0 0 ${S} ${S}`}
      style={{ overflow: "visible" }}
    >
      {/* Grid rings */}
      {[30, 60, 100].map((pct) => (
        <polygon
          key={pct}
          points={angles.map((a) => pt(a, pct).join(",")).join(" ")}
          fill="none"
          stroke="var(--line)"
          strokeWidth="0.75"
        />
      ))}
      {/* Axis lines */}
      {angles.map((a, i) => {
        const [x, y] = pt(a, 100);
        return (
          <line
            key={i}
            x1={cx}
            y1={cy}
            x2={x}
            y2={y}
            stroke="var(--line)"
            strokeWidth="0.75"
          />
        );
      })}
      {/* Score fill */}
      <polygon
        points={scorePoints.map((p) => p.join(",")).join(" ")}
        fill="rgba(91,106,63,0.2)"
        stroke="var(--olive)"
        strokeWidth="1.5"
        strokeLinejoin="round"
      />
      {/* Dots on score */}
      {scorePoints.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="3" fill="var(--olive)" />
      ))}
      {/* Labels */}
      {RADAR_AXES.map(({ label }, i) => {
        const labelR = maxR + 17;
        const [x, y] = [
          cx + labelR * Math.cos(angles[i]),
          cy + labelR * Math.sin(angles[i]),
        ];
        return (
          <text
            key={i}
            x={x}
            y={y}
            textAnchor="middle"
            dominantBaseline="middle"
            fontSize="8.5"
            fill="var(--ink-soft)"
            fontFamily="'JetBrains Mono', monospace"
            style={{ textTransform: "uppercase", letterSpacing: "0.08em" }}
          >
            {label}
          </text>
        );
      })}
    </svg>
  );
}

function getStyleTip(p) {
  const prefs = p.stylePrefs || [];
  if (!prefs.length)
    return "Complete your style profile to receive personalised tips on building a wardrobe that truly works for you.";
  if (prefs.includes("minimalist") && prefs.includes("casual"))
    return '"Your minimalist-casual profile thrives on quality basics. Invest in a few well-cut neutral pieces — they will carry 80% of your wardrobe without effort."';
  if (prefs.includes("formal") && prefs.includes("casual"))
    return '"You bridge formal and casual naturally. A well-fitted blazer in a neutral tone is your most versatile single purchase."';
  if (prefs.includes("bohemian") || prefs.includes("romantic"))
    return '"Your expressive style loves texture and movement. Layer patterns at different scales — one bold, one subtle — to add depth without chaos."';
  if (prefs.includes("minimalist"))
    return '"Your clean aesthetic is powerful. One unexpected texture each season keeps minimalism from feeling flat."';
  if (prefs.includes("sporty") || prefs.includes("streetwear"))
    return '"The best street looks nail contrast — pair an oversized piece with something fitted. Proportion is everything."';
  return '"A great wardrobe is one you dress from without thinking. Keep editing — the pieces that stay are the ones that truly belong."';
}

function styleScore(prefs) {
  if (!prefs?.length) return null;
  return Math.min(58 + prefs.length * 7, 94);
}

export default function Profile() {
  const navigate = useNavigate();
  const { profile, wardrobeCount } = useProfile();

  const p = profile || {};
  const colors = PALETTE_COLORS[p.colorPalette] || null;
  const score = styleScore(p.stylePrefs);

  // ── Edit-mode state ───────────────────────────────────────────────────────
  const [editing, setEditing] = useState(false);
  const [dbName, setDbName] = useState(p.name || "");
  const [gender, setGender] = useState(p.gender || "");
  const [bodyType, setBodyType] = useState(p.bodyType || p.body_type || "");
  const [heightFt, setHeightFt] = useState(p.height_ft || "");
  const [heightCm, setHeightCm] = useState(p.height_cm || "");
  const [sizeTops, setSizeTops] = useState(p.size_tops || "");
  const [sizeBottoms, setSizeBottoms] = useState(p.size_bottoms || "");
  const [sizeShoes, setSizeShoes] = useState(p.size_shoes || "");
  const [savedSnapshot, setSavedSnapshot] = useState({
    name: p.name || "",
    gender: p.gender || "",
    bodyType: p.bodyType || p.body_type || "",
    sizeTops: p.size_tops || "",
    sizeBottoms: p.size_bottoms || "",
    sizeShoes: p.size_shoes || "",
  });
  const [photoUrl, setPhotoUrl] = useState(
    p.profilePhotoUrl || p.profile_photo_url || null,
  );
  const [photoUploading, setPhotoUploading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const fileInputRef = useRef(null);

  // Sync name from profile (set by onboarding) into the editable field
  // only when the field is still empty — never overwrite user edits.
  useEffect(() => {
    if (!dbName && p.name) setDbName(p.name);
  }, [p.name]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    apiGet("/profile")
      .then((data) => {
        if (data) {
          const snap = {
            name: data.name || p.name || "",
            gender: data.gender || p.gender || "",
            bodyType: data.body_type || p.bodyType || p.body_type || "",
            sizeTops: data.size_tops || p.size_tops || "",
            sizeBottoms: data.size_bottoms || p.size_bottoms || "",
            sizeShoes: data.size_shoes || p.size_shoes || "",
          };
          setDbName(snap.name);
          setGender(snap.gender);
          setBodyType(snap.bodyType);
          setHeightFt(data.height_ft || p.height_ft || "");
          setHeightCm(data.height_cm || p.height_cm || "");
          setSizeTops(snap.sizeTops);
          setSizeBottoms(snap.sizeBottoms);
          setSizeShoes(snap.sizeShoes);
          setPhotoUrl(data.profile_photo_url || p.profilePhotoUrl || null);
          setSavedSnapshot(snap);
        }
      })
      .catch(() => {});
  }, []);

  async function handleSave() {
    setSaving(true);
    try {
      await apiPatch("/profile", {
        name: dbName,
        gender,
        bodyType,
        heightFt,
        heightCm,
        sizeTops,
        sizeBottoms,
        sizeShoes,
      });
      const sess = getProfile() || {};
      setProfile({ ...sess, gender, bodyType, name: dbName });
      setSavedSnapshot({ name: dbName, gender, bodyType, sizeTops, sizeBottoms, sizeShoes });
      setSaved(true);
      setTimeout(() => setSaved(false), 1800);
    } catch (_) {
    } finally {
      setSaving(false);
    }
  }

  async function handlePhotoUpload(e) {
    const file = e.target.files?.[0];
    if (!file) return;
    setPhotoUploading(true);
    const fd = new FormData();
    fd.append("photo", file);
    try {
      const res = await apiUpload("/profile/photo", fd);
      setPhotoUrl(res.profile_photo_url);
      const sess = getProfile() || {};
      setProfile({ ...sess, profile_photo_url: res.profile_photo_url });
    } catch (_) {
    } finally {
      setPhotoUploading(false);
    }
  }

  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        height: "100%",
        background: "var(--paper)",
      }}
    >
      {/* Scrollable body */}
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div
          style={{
            maxWidth: "600px",
            margin: "0 auto",
            padding: "clamp(28px, 4vw, 48px) clamp(24px, 4vw, 56px) 48px",
          }}
        >
          {/* ── Page header ── */}
          <div style={{ textAlign: "center", marginBottom: "28px" }}>
            <h2
              style={{
                fontFamily: "'Fraunces', serif",
                fontWeight: 300,
                fontSize: "clamp(30px, 4vw, 42px)",
                color: "var(--ink)",
                margin: "0 0 10px",
              }}
            >
              Your profile
            </h2>
            <p
              style={{
                fontFamily: "'Inter Tight', sans-serif",
                fontSize: "13px",
                color: "var(--muted)",
                lineHeight: 1.6,
                margin: "0 0 20px",
              }}
            >
              Your style identity, body details, and wardrobe preferences.
            </p>
            <hr
              style={{
                border: "none",
                borderTop: "1px solid var(--line)",
                margin: 0,
              }}
            />
          </div>
          {/* ── Profile photo ── */}
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "12px",
              padding: "28px 0 32px",
            }}
          >
            <div
              style={{
                width: "250px",
                aspectRatio: "1/1",
                borderRadius: "50%",
                overflow: "hidden",
                border: "3px solid var(--line)",
                background: "var(--cream-deep)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                boxShadow: "0 4px 24px rgba(0,0,0,0.10)",
              }}
            >
              {photoUrl || p.profilePhotoUrl ? (
                <img
                  src={photoUrl || p.profilePhotoUrl}
                  alt={dbName || p.name || "Profile"}
                  style={{
                    width: "100%",
                    height: "100%",
                    objectFit: "contain",
                    display: "block",
                  }}
                />
              ) : (
                <div
                  style={{
                    fontFamily: "'Fraunces', serif",
                    fontSize: "40px",
                    fontStyle: "italic",
                    color: "var(--muted)",
                  }}
                >
                  {(dbName || p.name || "U")[0]?.toUpperCase()}
                </div>
              )}
            </div>
            {dbName || p.name ? (
              <div
                style={{
                  fontFamily: "'Fraunces', serif",
                  fontWeight: 300,
                  fontSize: "20px",
                  color: "var(--ink)",
                  letterSpacing: "-0.01em",
                }}
              >
                {dbName || p.name}
              </div>
            ) : null}
          </div>

          {/* ── About you (view) ── */}
          {!editing && (
            <div>
              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 28px",
                }}
              />

              {/* Full name */}
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                  gap: "24px",
                  marginBottom: "24px",
                }}
              >
                <div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "14px",
                      fontWeight: 600,
                      color: "var(--ink)",
                    }}
                  >
                    Full name
                  </div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "11px",
                      color: "var(--muted)",
                      marginTop: "3px",
                    }}
                  >
                    Used for greetings and your account
                  </div>
                </div>
                <input
                  value={dbName}
                  onChange={(e) => setDbName(e.target.value)}
                  onBlur={() =>
                    apiPatch("/profile", { name: dbName }).catch(() => {})
                  }
                  placeholder="Your name"
                  style={{
                    width: "220px",
                    flexShrink: 0,
                    border: "1.5px solid var(--line)",
                    borderRadius: "8px",
                    padding: "10px 14px",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    color: "var(--ink)",
                    background: "white",
                    outline: "none",
                  }}
                />
              </div>

              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 24px",
                }}
              />

              {/* Gender */}
              <div style={{ marginBottom: "28px" }}>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "14px",
                    fontWeight: 600,
                    color: "var(--ink)",
                    marginBottom: "4px",
                  }}
                >
                  Gender
                </div>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "11px",
                    color: "var(--muted)",
                    marginBottom: "12px",
                  }}
                >
                  Affects how the AI renders garments on you
                </div>
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  {GENDERS.map((g) => {
                    const active = gender.toLowerCase() === g.toLowerCase();
                    return (
                      <button
                        key={g}
                        type="button"
                        onClick={() => setGender(active ? "" : g)}
                        style={{
                          padding: "8px 18px",
                          borderRadius: "100px",
                          border: active
                            ? "1.5px solid var(--ink)"
                            : "1.5px solid var(--line)",
                          background: active ? "var(--ink)" : "transparent",
                          fontFamily: "'Inter Tight', sans-serif",
                          fontSize: "13px",
                          color: active ? "var(--paper)" : "var(--ink-soft)",
                          fontWeight: active ? 600 : 400,
                          cursor: "pointer",
                        }}
                      >
                        {g}
                      </button>
                    );
                  })}
                </div>
              </div>

              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 24px",
                }}
              />

              {/* Body type */}
              <div style={{ marginBottom: "28px" }}>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "14px",
                    fontWeight: 600,
                    color: "var(--ink)",
                    marginBottom: "4px",
                  }}
                >
                  Body type
                </div>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "11px",
                    color: "var(--muted)",
                    marginBottom: "14px",
                  }}
                >
                  Used for AI styling recommendations only
                </div>
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  {BODY_TYPES.map((bt) => {
                    const active = bodyType === bt.key;
                    return (
                      <button
                        key={bt.key}
                        type="button"
                        onClick={() => setBodyType(active ? "" : bt.key)}
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          alignItems: "center",
                          gap: "6px",
                          padding: "12px 14px",
                          borderRadius: "12px",
                          border: active
                            ? "2px solid var(--terracotta)"
                            : "1.5px solid var(--line)",
                          background: active ? "rgba(194,86,58,0.07)" : "white",
                          color: active
                            ? "var(--terracotta)"
                            : "var(--ink-soft)",
                          minWidth: "64px",
                          cursor: "pointer",
                        }}
                      >
                        <span style={{ opacity: active ? 1 : 0.35 }}>
                          {bt.svg}
                        </span>
                        <span
                          style={{
                            fontFamily: "'Inter Tight', sans-serif",
                            fontSize: "11px",
                            fontWeight: active ? 600 : 400,
                          }}
                        >
                          {bt.label}
                        </span>
                      </button>
                    );
                  })}
                </div>
              </div>

              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 24px",
                }}
              />

              {/* Sizes */}
              <div style={{ marginBottom: "32px" }}>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "14px",
                    fontWeight: 600,
                    color: "var(--ink)",
                    marginBottom: "4px",
                  }}
                >
                  Sizes
                </div>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "11px",
                    color: "var(--muted)",
                    marginBottom: "16px",
                  }}
                >
                  Used to personalise fit recommendations
                </div>
                <div style={{ display: "flex", gap: "16px" }}>
                  {[
                    {
                      label: "Tops",
                      hint: "XS / S / M / L / XL",
                      val: sizeTops,
                      set: setSizeTops,
                      ph: "M",
                    },
                    {
                      label: "Bottoms",
                      hint: "Numeric or sized",
                      val: sizeBottoms,
                      set: setSizeBottoms,
                      ph: "28",
                    },
                    {
                      label: "Shoes",
                      hint: "US sizing",
                      val: sizeShoes,
                      set: setSizeShoes,
                      ph: "8",
                    },
                  ].map(({ label, hint, val, set, ph }) => (
                    <div key={label} style={{ flex: 1 }}>
                      <div
                        style={{
                          fontFamily: "'Inter Tight', sans-serif",
                          fontSize: "12px",
                          fontWeight: 600,
                          color: "var(--ink)",
                          marginBottom: "2px",
                        }}
                      >
                        {label}
                      </div>
                      <div
                        style={{
                          fontFamily: "'Inter Tight', sans-serif",
                          fontSize: "10px",
                          color: "var(--muted)",
                          marginBottom: "8px",
                        }}
                      >
                        {hint}
                      </div>
                      <input
                        value={val}
                        onChange={(e) => set(e.target.value)}
                        placeholder={ph}
                        style={{
                          width: "100%",
                          boxSizing: "border-box",
                          border: "1.5px solid var(--line)",
                          borderRadius: "8px",
                          padding: "10px 12px",
                          fontFamily: "'Inter Tight', sans-serif",
                          fontSize: "13px",
                          color: "var(--ink)",
                          background: "white",
                          outline: "none",
                          textAlign: "center",
                        }}
                      />
                    </div>
                  ))}
                </div>
              </div>

              {/* Save / Cancel */}
              <div
                style={{
                  display: "flex",
                  gap: "12px",
                  justifyContent: "flex-end",
                  paddingTop: "8px",
                  borderTop: "1px solid var(--line)",
                }}
              >
                <button
                  onClick={() => {
                    setDbName(savedSnapshot.name);
                    setGender(savedSnapshot.gender);
                    setBodyType(savedSnapshot.bodyType);
                    setSizeTops(savedSnapshot.sizeTops);
                    setSizeBottoms(savedSnapshot.sizeBottoms);
                    setSizeShoes(savedSnapshot.sizeShoes);
                  }}
                  style={{
                    padding: "11px 24px",
                    borderRadius: "100px",
                    border: "1.5px solid var(--line)",
                    background: "transparent",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    fontWeight: 500,
                    color: "var(--ink-soft)",
                    cursor: "pointer",
                  }}
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  style={{
                    padding: "11px 28px",
                    borderRadius: "100px",
                    border: "none",
                    background: saved ? "#5B6A3F" : "var(--ink)",
                    color: "var(--paper)",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    fontWeight: 600,
                    cursor: saving ? "wait" : "pointer",
                    letterSpacing: "0.01em",
                    transition: "background 0.2s",
                  }}
                >
                  {saved ? "Saved ✓" : saving ? "Saving…" : "Save changes"}
                </button>
              </div>
            </div>
          )}

          {/* ── Edit form ── */}
          {editing && (
            <div
              style={{
                padding: "24px 0",
                borderBottom: "1px solid var(--line)",
              }}
            >
              {/* Header */}
              <div style={{ marginBottom: "20px" }}>
                <div
                  style={{
                    fontFamily: "'Fraunces', serif",
                    fontWeight: 400,
                    fontSize: "24px",
                    color: "var(--ink)",
                    marginBottom: "6px",
                  }}
                >
                  About you
                </div>
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    color: "var(--muted)",
                    lineHeight: 1.5,
                  }}
                >
                  These details help the AI render try-ons and style outfits
                  that actually work for your body.
                </div>
              </div>

              {/* Why we ask */}
              <div
                style={{
                  background: "var(--cream-deep)",
                  borderLeft: "3px solid var(--terracotta)",
                  borderRadius: "8px",
                  padding: "12px 14px",
                  marginBottom: "24px",
                }}
              >
                <div
                  style={{
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "12px",
                    color: "var(--ink)",
                    lineHeight: 1.6,
                  }}
                >
                  <strong>★ Why we ask:</strong> Gender helps Perfect Corp's
                  try-on AI position garments correctly on your body. Body type
                  helps pick styling recommendations that flatter your
                  proportions. None of this is shared, ever.
                </div>
              </div>

              {/* ── IDENTITY ── */}
              <div style={{ ...monoLabel, marginBottom: "12px" }}>Identity</div>
              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 20px",
                }}
              />

              {/* Full name */}
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                  gap: "24px",
                  marginBottom: "20px",
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      fontWeight: 600,
                      color: "var(--ink)",
                    }}
                  >
                    Full name
                  </div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "11px",
                      color: "var(--muted)",
                      marginTop: "2px",
                    }}
                  >
                    Used for greetings and your account
                  </div>
                </div>
                <input
                  value={dbName}
                  onChange={(e) => setDbName(e.target.value)}
                  placeholder="Your name"
                  style={{
                    width: "220px",
                    flexShrink: 0,
                    border: "1.5px solid var(--line)",
                    borderRadius: "8px",
                    padding: "9px 12px",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    color: "var(--ink)",
                    background: "white",
                    outline: "none",
                  }}
                />
              </div>

              {/* Gender */}
              <div
                style={{
                  display: "flex",
                  alignItems: "flex-start",
                  justifyContent: "space-between",
                  gap: "24px",
                  marginBottom: "20px",
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      fontWeight: 600,
                      color: "var(--ink)",
                    }}
                  >
                    Gender
                  </div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "11px",
                      color: "var(--muted)",
                      marginTop: "2px",
                      maxWidth: "160px",
                      lineHeight: 1.4,
                    }}
                  >
                    Affects how the AI renders garments on you
                  </div>
                </div>
                <div
                  style={{
                    display: "flex",
                    flexWrap: "wrap",
                    gap: "8px",
                    justifyContent: "flex-end",
                  }}
                >
                  {GENDERS.map((g) => (
                    <button
                      key={g}
                      type="button"
                      onClick={() => setGender(gender === g ? "" : g)}
                      style={{
                        padding: "7px 16px",
                        borderRadius: "100px",
                        border:
                          gender === g ? "none" : "1.5px solid var(--line)",
                        background: gender === g ? "var(--ink)" : "white",
                        color: gender === g ? "white" : "var(--ink-soft)",
                        fontFamily: "'Inter Tight', sans-serif",
                        fontSize: "13px",
                        fontWeight: gender === g ? 600 : 400,
                        cursor: "pointer",
                      }}
                    >
                      {g}
                    </button>
                  ))}
                </div>
              </div>

              {/* ── BODY ── */}
              <div
                style={{ ...monoLabel, marginBottom: "12px", marginTop: "8px" }}
              >
                Body
              </div>
              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 20px",
                }}
              />

              {/* Body type */}
              <div style={{ marginBottom: "20px" }}>
                <div
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                    marginBottom: "12px",
                  }}
                >
                  <div>
                    <div
                      style={{
                        fontFamily: "'Inter Tight', sans-serif",
                        fontSize: "13px",
                        fontWeight: 600,
                        color: "var(--ink)",
                      }}
                    >
                      Body type
                    </div>
                    <div
                      style={{
                        fontFamily: "'Inter Tight', sans-serif",
                        fontSize: "11px",
                        color: "var(--muted)",
                        marginTop: "2px",
                      }}
                    >
                      Used for AI styling recommendations only
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                  {BODY_TYPES.map((bt) => {
                    const active = bodyType === bt.key;
                    return (
                      <button
                        key={bt.key}
                        type="button"
                        title={bt.desc}
                        onClick={() =>
                          setBodyType(bodyType === bt.key ? "" : bt.key)
                        }
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          alignItems: "center",
                          gap: "4px",
                          padding: "10px 14px",
                          borderRadius: "10px",
                          border: active
                            ? "2px solid var(--terracotta)"
                            : "1.5px solid var(--line)",
                          background: active ? "rgba(194,86,58,0.07)" : "white",
                          color: active
                            ? "var(--terracotta)"
                            : "var(--ink-soft)",
                          cursor: "pointer",
                          minWidth: "60px",
                        }}
                      >
                        <span style={{ opacity: active ? 1 : 0.5 }}>
                          {bt.svg}
                        </span>
                        <span
                          style={{
                            fontFamily: "'Inter Tight', sans-serif",
                            fontSize: "11px",
                            fontWeight: active ? 600 : 400,
                          }}
                        >
                          {bt.label}
                        </span>
                      </button>
                    );
                  })}
                </div>
              </div>

              {/* Height */}
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                  gap: "24px",
                  marginBottom: "20px",
                }}
              >
                <div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      fontWeight: 600,
                      color: "var(--ink)",
                    }}
                  >
                    Height
                  </div>
                  <div
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "11px",
                      color: "var(--muted)",
                      marginTop: "2px",
                      maxWidth: "160px",
                      lineHeight: 1.4,
                    }}
                  >
                    Optional — helps with proportional fit guidance
                  </div>
                </div>
                <div
                  style={{ display: "flex", gap: "8px", alignItems: "center" }}
                >
                  <input
                    value={heightFt}
                    onChange={(e) => setHeightFt(e.target.value)}
                    placeholder={`5'5"`}
                    style={{
                      width: "72px",
                      border: "1.5px solid var(--line)",
                      borderRadius: "8px",
                      padding: "9px 10px",
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      color: "var(--ink)",
                      background: "white",
                      outline: "none",
                    }}
                  />
                  <span
                    style={{
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "12px",
                      color: "var(--muted)",
                    }}
                  >
                    or
                  </span>
                  <input
                    value={heightCm}
                    onChange={(e) => setHeightCm(e.target.value)}
                    placeholder="165 cm"
                    style={{
                      width: "80px",
                      border: "1.5px solid var(--line)",
                      borderRadius: "8px",
                      padding: "9px 10px",
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      color: "var(--ink)",
                      background: "white",
                      outline: "none",
                    }}
                  />
                </div>
              </div>

              {/* ── SIZES ── */}
              <div
                style={{ ...monoLabel, marginBottom: "12px", marginTop: "8px" }}
              >
                Sizes
              </div>
              <hr
                style={{
                  border: "none",
                  borderTop: "1px solid var(--line)",
                  margin: "0 0 4px",
                }}
              />

              {[
                {
                  label: "Tops",
                  hint: "XS / S / M / L / XL / numeric",
                  val: sizeTops,
                  set: setSizeTops,
                  ph: "M",
                },
                {
                  label: "Bottoms",
                  hint: "Numeric or sized",
                  val: sizeBottoms,
                  set: setSizeBottoms,
                  ph: "28",
                },
                {
                  label: "Shoes",
                  hint: "US sizing",
                  val: sizeShoes,
                  set: setSizeShoes,
                  ph: "8",
                },
              ].map(({ label, hint, val, set, ph }) => (
                <div
                  key={label}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "space-between",
                    gap: "24px",
                    padding: "14px 0",
                    borderBottom: "1px solid var(--line)",
                  }}
                >
                  <div>
                    <div
                      style={{
                        fontFamily: "'Inter Tight', sans-serif",
                        fontSize: "13px",
                        fontWeight: 600,
                        color: "var(--ink)",
                      }}
                    >
                      {label}
                    </div>
                    <div
                      style={{
                        fontFamily: "'Inter Tight', sans-serif",
                        fontSize: "11px",
                        color: "var(--muted)",
                        marginTop: "2px",
                      }}
                    >
                      {hint}
                    </div>
                  </div>
                  <input
                    value={val}
                    onChange={(e) => set(e.target.value)}
                    placeholder={ph}
                    style={{
                      width: "100px",
                      border: "1.5px solid var(--line)",
                      borderRadius: "8px",
                      padding: "8px 12px",
                      fontFamily: "'Inter Tight', sans-serif",
                      fontSize: "13px",
                      color: "var(--ink)",
                      background: "white",
                      outline: "none",
                    }}
                  />
                </div>
              ))}

              {/* Actions */}
              <div
                style={{
                  display: "flex",
                  gap: "10px",
                  justifyContent: "flex-end",
                  marginTop: "24px",
                }}
              >
                <button
                  onClick={() => setEditing(false)}
                  style={{
                    padding: "10px 22px",
                    borderRadius: "10px",
                    border: "1.5px solid var(--line)",
                    background: "white",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    color: "var(--ink-soft)",
                    cursor: "pointer",
                  }}
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  style={{
                    padding: "10px 22px",
                    borderRadius: "10px",
                    border: "none",
                    background: saved ? "#5B6A3F" : "var(--ink)",
                    color: "white",
                    fontFamily: "'Inter Tight', sans-serif",
                    fontSize: "13px",
                    fontWeight: 600,
                    cursor: saving ? "wait" : "pointer",
                    display: "flex",
                    alignItems: "center",
                    gap: "6px",
                  }}
                >
                  {saved ? "Saved ✓" : saving ? "Saving…" : "Save changes ✓"}
                </button>
              </div>
            </div>
          )}

          {/* ── My Style ── */}
          {p.stylePrefs?.length > 0 && (
            <Section label="My Style">
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {p.stylePrefs.map((k) => (
                  <span
                    key={k}
                    style={{
                      fontFamily: "'JetBrains Mono', monospace",
                      fontSize: "9px",
                      letterSpacing: "0.12em",
                      textTransform: "uppercase",
                      padding: "6px 12px",
                      borderRadius: "100px",
                      background: "var(--ink)",
                      color: "var(--paper)",
                    }}
                  >
                    {STYLE_LABELS[k] || k}
                  </span>
                ))}
              </div>
            </Section>
          )}

          {/* ── Style Tip ── */}
          {p.stylePrefs?.length > 0 && (
            <Section label="Style Tip">
              <div
                style={{
                  background: "var(--cream-deep)",
                  borderRadius: "12px",
                  padding: "16px 18px",
                  borderLeft: "3px solid var(--terracotta)",
                }}
              >
                <div
                  style={{
                    ...monoLabel,
                    color: "var(--terracotta)",
                    marginBottom: "8px",
                  }}
                >
                  ★ Your tip
                </div>
                <p
                  style={{
                    fontFamily: "'Fraunces', serif",
                    fontStyle: "italic",
                    fontWeight: 300,
                    fontSize: "15px",
                    lineHeight: 1.55,
                    color: "var(--ink-soft)",
                  }}
                >
                  {getStyleTip(p)}
                </p>
              </div>
            </Section>
          )}

          {!profile && (
            <div style={{ textAlign: "center", padding: "48px 0" }}>
              <div
                style={{
                  fontFamily: "'Fraunces', serif",
                  fontSize: "64px",
                  fontStyle: "italic",
                  color: "var(--line)",
                  marginBottom: "16px",
                }}
              >
                C.
              </div>
              <p
                style={{
                  fontFamily: "'Fraunces', serif",
                  fontStyle: "italic",
                  fontSize: "16px",
                  color: "var(--muted)",
                  marginBottom: "24px",
                }}
              >
                You haven't set up your profile yet.
              </p>
              <button
                onClick={() => navigate("/onboarding")}
                style={{
                  background: "var(--ink)",
                  color: "var(--paper)",
                  border: "none",
                  borderRadius: "100px",
                  padding: "14px 28px",
                  fontFamily: "'JetBrains Mono', monospace",
                  fontSize: "10px",
                  fontWeight: 500,
                  letterSpacing: "0.25em",
                  textTransform: "uppercase",
                  cursor: "pointer",
                }}
              >
                Set up profile →
              </button>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}

function Section({ label, children }) {
  return (
    <div style={{ padding: "20px 0", borderBottom: "1px solid var(--line)" }}>
      <div style={{ fontFamily: "'Inter Tight', sans-serif", fontSize: "14px", fontWeight: 600, color: "var(--ink)", marginBottom: "12px" }}>{label}</div>
      {children}
    </div>
  );
}

const monoLabel = {
  fontFamily: "'JetBrains Mono', monospace",
  fontSize: "9px",
  letterSpacing: "0.22em",
  textTransform: "uppercase",
  color: "var(--muted)",
};
