...
function readData() {
  if (!fs.existsSync(DATA_FILE))
    return { user: {}, wardrobe: [], wishlist: [], users: [] };
...
app.post("/api/wardrobe/from-url", (req, res) => {
...
async function startPerfectCorpRender({
  srcFileUrl,
  srcId,
  refFileUrl,
  ...
  if (!json.data?.task_id)
    throw new Error(`Perfect Corp start failed: ${JSON.stringify(json)}`);
...
async function pollPerfectCorpTask(taskId) {
  for (let i = 0; i < 20; i++) {
    ...
    if (task_status === "success")
      return { url: results.url, dst_id: results.dst_id };
...
async function renderOutfitChain(
  profilePhotoUrl,
  newItemUrl,
  additionalItems = [],
  garmentCategory = "auto",
) {
  let output = await perfectCorpRender({
    srcFileUrl: profilePhotoUrl,
    refFileUrl: newItemUrl,
    garmentCategory,
  });
  for (const item of additionalItems) {
    const itemCategory = toPerfectCorpCategory(item.category);
    output = await perfectCorpRender({
      srcId: output.dst_id,
      refFileUrl: item.image_url,
      garmentCategory: itemCategory,
    });
  }
  return output.url;
}
...
async function buildPairingWithGroq(
  newItemUrl,
  profilePhotoUrl,
  category,
  price,
  ...
  if (wardrobe.length === 0) {
    return {
      combinations: [],
      honest_assessment:
        "Add items to your wardrobe to get outfit pairing suggestions.",
    };
  }
...
4. Focus on the new garment only — assess honestly whether it suits the person's body type and skin tone based on the image. Do NOT describe or reference what the person is currently wearing. Address the user directly as "you". Be specific and kind but truthful.
...
6. Pick 2 outfit combinations from their wardrobe that would work well with this new item. Never pair the new item with a wardrobe item of the same category (e.g. do not pair a dress with another dress, a top with another top, pants with pants). Only combine complementary categories (e.g. dress + shoes + bag, top + bottom + outerwear).
...
Return ONLY valid JSON, no markdown, no code blocks:
{
  ...
  "honest_assessment": "2-3 sentences: whether this garment suits your body type and skin tone, and whether it fills a real gap or duplicates something you already own. Never reference what the person is currently wearing."
}`;
  ...
  // Enforce no same-category pairings — filter out wardrobe items that share
  // the same category as the new item from every combination
  const newCategory = (parsed.detected_category || category || '').toLowerCase()
  if (parsed.combinations && newCategory) {
    parsed.combinations = parsed.combinations.map(combo => {
      const filteredIds = (combo.wardrobe_item_ids || []).filter(id => {
        const wardrobeItem = wardrobe.find(w => w.id === id)
        return wardrobeItem && wardrobeItem.category.toLowerCase() !== newCategory
      })
      return { ...combo, wardrobe_item_ids: filteredIds }
    }).filter(combo => combo.wardrobe_item_ids.length > 0)
  }

  fs.writeFileSync(
    path.join(__dirname, "llm_response.json"),
    JSON.stringify({ timestamp: new Date().toISOString(), raw: text, parsed }, null, 2)
  );
  return parsed;
}
  ...
  try {
    ...
    if (!profilePhotoUrl) {
      return res.status(400).json({
        error: "No profile photo found. Please complete onboarding first.",
      });
    }
    ...
    let combinations = [],
      ...
      similar_owned = "";
    try {
      const pairing = await buildPairingWithGroq(
        newItemUrl,
        profilePhotoUrl,
        category,
        price,
      ...
      honest_assessment =
        "Style analysis unavailable — check your GROQ_API_KEY.";
    }
    ...
    try {
      soloRenderUrl = await renderOutfitChain(
        profilePhotoUrl,
        newItemUrl,
        [],
        pcCategory,
    ...
    const enrichedCombos = await Promise.all(
      combinations.slice(0, 2).map(async (combo) => {
        ...
        try {
          compositeUrl = await renderOutfitChain(
            profilePhotoUrl,
            newItemUrl,
            comboItems,
            pcCategory,
          ...
          console.error(
            `Combo render failed for "${combo.name}":`,
            err.message,
          );
        }
...
app.listen(PORT, () =>
  console.log(`Closet API running on http://localhost:${PORT}`),
);
